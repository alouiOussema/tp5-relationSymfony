...
class InstitutController extends AbstractController
{
     ...
     */
    public function new(Request $request, InstitutRepository $institutRepository): Response
    {
     ...
     */
    public function delete(Request $request, Institut $institut, InstitutRepository $institutRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$institut->getId(), $request->request->get('_token'))) {
            $institutRepository->remove($institut, true);
        }
